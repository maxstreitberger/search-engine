...
TEST_CASE("Document Store can", "[store]") {
    ...
    std::set<docmeta::DocumentMeta> documents = { docmeta::DocumentMeta(1, "Hello, World!", CRAWLER_TESTING_ROOT_DIR "/testing-documents/demo.txt") };
    ...
    SECTION("retrieve documents in the store.") {
        ...
        std::set<docmeta::DocumentMeta> currentStore = store.loadDocuments(store.store_path);
    ...
    SECTION("retrieve documents found by crawler.") {
        ...
        std::set<docmeta::DocumentMeta> doc_found_by_crawler = store.loadDocuments(store.crawled_docs_path);
    ...
    SECTION("check for new documents.") {
        ...
        std::vector<docmeta::DocumentMeta> foundNewDocuments = store.checkForChanges(&resultingStore, documents).first;

        REQUIRE( expectedResult == foundNewDocuments );
    }

    SECTION("check for updated documents.") {
        std::set<docmeta::DocumentMeta> resultingStore = documents;
        std::set<docmeta::DocumentMeta> toUpdateDocument = { docmeta::DocumentMeta(1, "Hello, World! How are you?", CRAWLER_TESTING_ROOT_DIR "/testing-documents/demo.txt") };
    ...
    SECTION("add new documments to the store.") {
        std::set<docmeta::DocumentMeta> expectedStore = { docmeta::DocumentMeta(1, "Hello, World!", CRAWLER_TESTING_ROOT_DIR "/testing-documents/demo.txt") };

        std::set<docmeta::DocumentMeta> resultingStore;
        DocStore store = DocStore(storePath, crawledDocsPath, repoPath);

        std::vector newDocumment = { docmeta::DocumentMeta(1, "Hello, World!", CRAWLER_TESTING_ROOT_DIR "/testing-documents/demo.txt") };
    ...
    SECTION("update documments in the store.") {
        std::set<docmeta::DocumentMeta> expectedStore = { docmeta::DocumentMeta(1, "Hello, World! How are you?", CRAWLER_TESTING_ROOT_DIR "/testing-documents/demo.txt") };

        std::set<docmeta::DocumentMeta> resultingStore = { docmeta::DocumentMeta(1, "Hello, World!", CRAWLER_TESTING_ROOT_DIR "/testing-documents/demo.txt") };
        DocStore store = DocStore(storePath, crawledDocsPath, repoPath);

        std::vector updatedDocument = { docmeta::DocumentMeta(1, "Hello, World! How are you?", CRAWLER_TESTING_ROOT_DIR "/testing-documents/demo.txt") };
    ...
    SECTION("write changes to repository.") {
        std::vector newAndChangedDocuments = { docmeta::DocumentMeta(1, "Hello, World! How are you?", CRAWLER_TESTING_ROOT_DIR "/testing-documents/demo.txt") };
        DocStore store = DocStore(storePath, crawledDocsPath, repoPath);
